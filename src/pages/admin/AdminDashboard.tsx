...
const AdminDashboard = () => {
  ...
  // Use the hook correctly - data is always an array from the hook
  const { 
    data: statsData, 
    loading: statsLoading, 
    error: statsError 
  } = useFirestoreCollection<StatsDoc>("dashboard_stats", []);
  
  const { 
    data: ordersData, 
    loading: ordersLoading, 
    error: ordersError 
  } = useFirestoreCollection<Order>("orders", [orderBy("date", "desc"), limit(5)]);
  ...
  // Ensure we have arrays to work with (though hook already returns array)
  const safeStatsData = Array.isArray(statsData) ? statsData : [];
  const safeOrdersData = Array.isArray(ordersData) ? ordersData : [];
  
  // Get first stats document or use fallback
  const s = safeStatsData.length > 0 ? safeStatsData[0] : fallbackStats[0];
  
  // Check if we should show fallback (error state)
  const showFallback = statsError !== null && safeStatsData.length === 0;
  ...
  // Show loading state for stats only if loading and no data
  if (statsLoading && safeStatsData.length === 0) {
    return (
  ...
  if (statsError && safeStatsData.length === 0 && !showFallback) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Low Stock Alerts - Make sure this component is safe */}
      <LowStockAlerts />
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
        ...
        {ordersLoading ? (
          ...
          <div className="px-6 py-12 text-center">
            ...
            <p className="text-muted-foreground text-sm mt-2">Using fallback data</p>
            <div className="mt-4">
              {fallbackOrders.map((order) => (
                <div key={order.id} className="text-left border-b p-4">
